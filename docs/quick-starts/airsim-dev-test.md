...
On Windows (Workstation A - with Project AirSim)
  ...
  ```ps
  New-NetFirewallRule -DisplayName "AirSim-Zenoh Bridge" -Direction Inbound -Protocol TCP -LocalPort 7447 -Action Allow
  ```
  4. Run the bridge:
  ```bash
...
Here's how to open port 7447 on Windows for the AirSim-Zenoh bridge:
...
```cmd
> netsh advfirewall firewall add rule name="AirSim-Zenoh Bridge" dir=in action=allow protocol=tcp localport=7447
```

## Verify the Rule

  Get-NetFirewallRule -DisplayName "AirSim-Zenoh Bridge"

## Test from Linux Workstation

  After opening the port, test connectivity from your Linux box:

  nc -zv <windows-ip> 7447
