 ...
 * **`Waypoint.h`**: A structure defining a target position, potentially with arrival tolerance and speed constraints.

   ```cpp
   struct Waypoint {
       float x, y, z;
       float yaw_target; // Desired orientation at waypoint
       // ... serialization methods
   };
   ```

 * **`ControlCommand.h`**: A structure for the low-level inputs the simulator expects (e.g., linear velocity and angular velocity).
 ...
 * **`PIDController.h`**: A generic class to implement Proportional-Integral-Derivative control logic.

 * **`WaypointFollower.h`**: A higher-level logic class that uses the PID controller to track a target point. It calculates the error between the current position (from Odometry) and the target (Waypoint) and outputs the desired `ControlCommand`.

#### C. Platform Interface (`platforms/drone_platform/`)
 ...
 * **`DroneController.hp`**: A class that wraps the `WaypointFollower` and manages the state of the drone (e.g., Armed, Taking Off, Flying, Landing).
...
| **Waypoint Target** | `autonomy/waypoint_topic` | `Waypoint` | Waypoint Planner | Platform Controller |
...
target_link_libraries(controller_node PRIVATE
   ...
   data_types
   control_algorithms
   drone_platform       # Links the high-level control logic
)
